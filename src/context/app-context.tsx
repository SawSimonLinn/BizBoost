...
export function AppProvider({ children }: { children: React.ReactNode }) {
    ...
    React.useEffect(() => {
        ...
        setActivePeriodId(getFromLocalStorage('bizboost_activePeriodId', mockPeriods[mockPeriods.length - 1].id));
        ...
        setPersonalExpenses(getFromLocalStorage('bizboost_personalExpenses', mockPersonalExpenses));
    ...
    React.useEffect(() => {
        if (isHydrated) {
            localStorage.setItem('bizboost_personalExpenses', JSON.stringify(personalExpenses));
        }
    ...
    const calculateMetricsForPeriod = (period: Period, config: FeeConfig, costs: StaffCost[]) => {
        ...
        const inventoryCostValue = period.inventoryCostType === 'percent'
    ...
    if (!isHydrated) {
        const defaultContext = {
            ...
            inventoryCostValue: mockPeriods[mockPeriods.length-1].inventoryCostType === 'percent' 
              ? (mockPeriods[mockPeriods.length-1].weeklySales.reduce((a,b) => a+b, 0) * mockPeriods[mockPeriods.length-1].inventoryCost) / 100
