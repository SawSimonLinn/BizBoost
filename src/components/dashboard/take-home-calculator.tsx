...
type LocalExpense = Omit<PersonalExpense, "amount"> & {
...
const CalculatorContent = ({
  ...
  const [localExpenses, setLocalExpenses] =
    React.useState<LocalExpense[]>(personalExpenses);

  React.useEffect(() => {
    setLocalExpenses(personalExpenses);
  }, [personalExpenses]);

  const totalPersonalExpenses = localExpenses.reduce(
    (acc, item) => acc + (parseFloat(item.amount as string) || 0),
    0
  );
  const remainingForSavings = netEarnings - totalPersonalExpenses;
  const goalAmount = parseFloat(savingsGoal as string) || 0;
  const monthsToGoal =
    goalAmount > 0 && remainingForSavings > 0
      ? Math.ceil(goalAmount / remainingForSavings)
      : 0;
    ...
    if (newItem && newAmount) {
      setPersonalExpenses([
        ...personalExpenses,
        {
          ...
          name: newItem,
          amount: parseFloat(newAmount as string) || 0,
        },
      ]);
      setNewItem("");
      setNewAmount("");
  ...
  return (
    <>
      <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
        ...
        <div className="p-4 bg-success/10 rounded-lg col-span-2 md:col-span-1">
                  ...
                  <TableCell>
                    <div className="relative max-w-[150px] ml-auto">
                      <Input
                        ...
                        value={item.amount}
                        onChange={(e) =>
                          handleLocalUpdate(item.id, "amount", e.target.value)
                        }
                        onBlur={(e) =>
                          handleBlur(item.id, "amount", e.target.value)
                        }
