...
const InputField = ({ id, label, value, onChange, unit, type = "number", className, disabled = false }: { id: string; label: string; value: number | string; onChange: (value: string) => void; unit: string, type?: string, className?: string, disabled?: boolean }) => (
  ...
  export function FeeConfigurator({ 
    feeConfig, onFeeChange, activePeriod, onPeriodChange, totalSales 
    ...
    const totalOtherExpenses = React.useMemo(() => {
      return activePeriod.otherExpenses.reduce((acc, curr) => acc + curr.amount, 0);
    ...
    return (
      <Card>
        <CardHeader>
          ...
          <CardDescription>{t('Adjust your period data and fee structure here to see real-time updates.')}</CardDescription>
        </CardHeader>
        <CardContent className="grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-4">
          <div className="space-y-4">
               <div className="space-y-1">
                   <Label htmlFor="total-sales" className="text-xs font-medium">{t('Total Monthly Sales')}</Label>
                   <div className="relative">
                      <Input 
                        id="total-sales"
                        type="number"
                      ...
                      <Dialog open={isWeeklySalesDialogOpen} onOpenChange={setIsWeeklySalesDialogOpen}>
                       ...
                       <DialogContent>
                         <DialogHeader>
                           <DialogTitle>{t('Enter Weekly Sales for')} {activePeriod.name}</DialogTitle>
                         </DialogHeader>
                         ...
                         <DialogFooter>
                           <Button type="button" variant="ghost" onClick={handleCancelWeeklySales}>{t('Cancel')}</Button>
                           <Button type="button" onClick={handleSaveWeeklySales}>{t('Save Changes')}</Button>
                         </DialogFooter>
                       </DialogContent>
                      </Dialog>
                   </div>
                 ...
                 <div className="space-y-1">
                    <Label htmlFor="inventory-cost" className="text-xs font-medium">{t('Inventory Cost')}</Label>
            ...
            <div className="space-y-4">
              <div className="space-y-1">
                <Label htmlFor="royalty-fee" className="text-xs font-medium">{t('Your Percentage (%)')}</Label>
                <div className="relative">
                    <Input
                    id="royalty-fee"
                    type="number"
                    value={feeConfig.royaltyPercent}
                    onChange={(e) => onFeeChange('royaltyPercent', parseFloat(e.target.value) || 0)}
                    className="pr-12 text-base font-semibold"
                    />
                    <span className="absolute inset-y-0 right-0 flex items-center pr-3 text-sm text-muted-foreground">%</span>
                </div>
              ...
              <div className="space-y-1">
                   <Label htmlFor="total-additional-fees" className="text-xs font-medium">{t('Additional Fees')}</Label>
                   <div className="relative">
                      <Input 
                      ...
                      <Dialog open={isFeesDialogOpen} onOpenChange={setIsFeesDialogOpen}>
                       ...
                       <DialogContent>
                         <DialogHeader>
                           <DialogTitle>{t('Edit Additional Fees')}</DialogTitle>
                         </DialogHeader>
                         <div className="space-y-2 rounded-lg border p-4 max-h-60 overflow-y-auto">
                            {localOtherExpenses.length === 0 && <p className="text-sm text-muted-foreground text-center py-4">{t('No fees added yet.')}</p>}
                            {localOtherExpenses.map(exp => (
                                <div key={exp.id} className="flex items-center gap-2">
                                    ...
                                    <Button variant="ghost" size="icon" onClick={() => handleRemoveExpense(exp.id)}>
                         ...
                         <form onSubmit={handleAddExpense} className="mt-4 flex items-end gap-2 border-t pt-4">
                            <div className="flex-grow">
                                 <Label htmlFor="new-expense-name" className="text-xs font-medium">{t('New Fee Name')}</Label>
                                 <Input id="new-expense-name" placeholder={t('e.g., Tech Fee')} value={newExpenseName} onChange={e => setNewExpenseName(e.target.value)} />
                            </div>
                            <div className="w-32">
                         ...
                         <DialogFooter>
                            <Button type="button" variant="ghost" onClick={() => setIsFeesDialogOpen(false)}>{t('Cancel')}</Button>
                           <Button type="button" onClick={handleSaveFees}>{t('Save Changes')}</Button>
                         </DialogFooter>
                       </DialogContent>
                      </Dialog>
                   </div>
