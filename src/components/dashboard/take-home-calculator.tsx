        ...
        <>
             <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                  <div className="p-4 bg-primary/10 rounded-lg">
                      ...
                      <div className="text-2xl font-bold text-primary">{formatCurrency(netEarnings)}</div>
                  </div>
                   <div className="p-4 bg-destructive/10 rounded-lg">
                      <Label>Total Personal Expenses</Label>
                      <div className="text-2xl font-bold text-destructive">{formatCurrency(totalPersonalExpenses)}</div>
                  </div>
                  <div className="p-4 bg-success/10 rounded-lg">
                      <Label>Remaining for Savings</Label>
                      <div className="text-2xl font-bold text-success">{formatCurrency(remainingForSavings)}</div>
                  </div>
              </div>
                          ...
                          <TableBody>
                              {personalExpenses.map(item => (
                                  <TableRow key={item.id}>
                                      ...
                                      <TableCell>
                                        <Button variant="ghost" size="icon" onClick={() => handleRemoveExpense(item.id)}>
                              ...
                              {personalExpenses.length === 0 && (
                                <TableRow>
                                  <TableCell colSpan={3} className="text-center text-muted-foreground py-8">No expenses added.</TableCell>
                                </TableRow>
                              )}
                          </TableBody>
                      </Table>
                  </div>
                   <form onSubmit={handleAddExpense} className="mt-4 flex items-end gap-2 border-t pt-4">
                      <div className="flex-grow">
                           <Label htmlFor="new-expense-name" className="text-xs font-medium">New Expense</Label>
                           <Input id="new-expense-name" placeholder="e.g., Groceries" value={newItem} onChange={e => setNewItem(e.target.value)} />
                      </div>
                      <div className="w-40">
                           <Label htmlFor="new-expense-amount" className="text-xs font-medium">Amount</Label>
                           <Input id="new-expense-amount" type="number" placeholder="$" value={newAmount} onChange={e => setNewAmount(e.target.value)} step="0.01" />
                      </div>
              ...
              <div className="mt-6 border-t pt-6">
                  <h4 className="font-semibold mb-2 flex items-center gap-2"><Target /> Savings Goal</h4>
...
export function TakeHomeCalculator({ netEarnings, personalExpenses, setPersonalExpenses, asCard = true }: TakeHomeCalculatorProps) {
        ...
        <Card>
           <CardHeader>
            <div className="flex items-center justify-between">
              <div>
                <CardTitle className="font-headline flex items-center gap-2"><PiggyBank />Personal Finances</CardTitle>
              ...
              <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
                ...
                <DialogContent className="max-w-3xl">
                  <DialogHeader>
                    <DialogTitle className="font-headline">Personal Take-Home Calculator</DialogTitle>
                    <DialogDescription>
          ...
          <CardContent>
             <div className="space-y-2">
                <div className="flex justify-between items-baseline">
                    <span className="text-sm text-muted-foreground">Net Earnings After Staff</span>
                    <span className="font-semibold text-lg text-primary">{formatCurrency(netEarnings)}</span>
                </div>
                 <div className="flex justify-between items-baseline">
                    <span className="text-sm text-muted-foreground">Personal Expenses</span>
                    <span className="font-semibold text-lg text-destructive">-{formatCurrency(totalPersonalExpenses)}</span>
                </div>
                 <div className="flex justify-between items-baseline border-t pt-2 mt-2">
                    ...
                    <span className="font-bold text-xl text-success">{formatCurrency(remainingForSavings)}</span>
                </div>
             </div>
          </CardContent>
        </Card>
  ...
  return (
    <Card>
        <CardHeader>
             <CardTitle className="font-headline flex items-center gap-2"><PiggyBank />Personal Finances</CardTitle>
            <CardDescription>Log your personal expenses to see what's left for savings.</CardDescription>
        </CardHeader>
        <CardContent>
             <CalculatorContent 
                netEarnings={netEarnings} 
